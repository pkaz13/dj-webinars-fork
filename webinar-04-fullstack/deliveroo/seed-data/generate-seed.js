...
// Generate employee details (one-to-one with employees)
function generateEmployeeDetails(employees) {
  return employees.map(emp => ({
    // id will be auto-generated by SERIAL, so omit it
    employee_id: emp.id,
    address: faker.location.streetAddress(),
    postal_code: faker.location.zipCode(),
    city: faker.location.city(),
    country: faker.location.country(),
    date_of_birth: faker.date.birthdate({ min: 18, max: 65, mode: 'age' }).toISOString().slice(0, 10),
...
function main() {
  ...
  const employeeDetails = generateEmployeeDetails(employees);
  ...
  sql += '\n';
  sql += toSQLInsert('employees_details', employeeDetails, [
    'employee_id', 'address', 'postal_code', 'city', 'country', 'date_of_birth', 'gender', 'nationality',
  ]);
  sql += '\n';
